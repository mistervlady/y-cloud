...
def add_message(author, message):
    ...
    try:
        msg_id = str(int(time.time() * 1000)) + ''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=7))
        timestamp = datetime.now().isoformat()
        ...
        return {'id': msg_id, 'author': author, 'message': message, 'timestamp': timestamp}
    except Exception as error:
